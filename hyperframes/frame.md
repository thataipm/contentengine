...
colors:
  # Light-theme flip, 2026-08-13 (reverses the same-day dark-theme remix above this comment
  # was originally written for -- direct instruction: "if video looks good, use light theme,
  # I don't mind the color preference, just make sure we make best retention holding sexy
  # video"). Ground is warm cream again, voice is dark ink, but the multi-accent colorful
  # system from the dark pass stays -- accents are NOT reverted to the original single
  # terracotta "coral", they're kept as a rotating purple/green/orange/blue set, each darkened
  # from its dark-ground-tuned value so it still clears real text/stroke contrast against a
  # light ground (see accents: below). "ink"/"cream"/"tile"/"coral"/"navy" keep their original
  # token NAMES (every component/rule doc below references these keys) -- only values changed.
  ink: "#141413"        # dark voice color (text on light ground)
  cream: "#FAF9F5"      # warm cream ground
  tile: "#EFE9DE"       # half-step darker than ground -- content-gathering surface
  tile-strong: "#ECE3D4" # a further half-step down from tile
  coral: "#6952D6"      # primary accent -- darkened purple (was #8C7CFF on dark ground),
    # computed for 4.5:1+ text contrast against cream; ACCENTS[0] below is the same value
  navy: "#1C1C22"       # code/terminal surface -- deliberately stays DARK even on a light
    # ground (a real terminal is dark regardless of page theme); see code-surface component
  navy-soft: "#202027"
  navy-elev: "#242429"

# Multi-accent rotation, not a single fixed accent: "coral" above is the default/primary
# voltage color, but per direct instruction ("keep everything colorful on top") this project
# rotates accent color per beat rather than rationing to one hue. Each value here is the
# light-ground-safe DARKENED counterpart of the dark-theme accent it replaces -- use these
# directly for text/stroke/border color; a fill-with-dark-text-on-top (e.g. a CTA pill) can
# still use the brighter un-darkened hue since contrast there comes from the dark text, not
# the fill itself:
accents: ["#6952D6", "#1F7A50", "#D97A1F", "#1C87A8"]  # purple, green, orange, blue (darkened)
ink-dim: "#6C6965"  # muted/secondary text on light ground -- a real solid dark gray (not a
  # low-alpha "ink"), verified via `npx hyperframes check`'s own real contrast checker against
  # the actual composited tile background (04-gap.html's tile labels, 2026-08-13). Use this,
  # never a low-alpha version of "ink" or the dark-ground-tuned "#9A9AA2", for any dimmed/
  # secondary text on a light ground -- alpha-blending dark text toward a light ground fails
  # contrast checks silently the same way the inverse (light text toward dark) does.
