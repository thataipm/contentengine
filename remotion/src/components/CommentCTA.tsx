...
const Star: React.FC<{ size: number; delay: number; frame: number; fps: number }> = ({ size, delay, frame, fps }) => {
  const p = springIn(frame, fps, delay);
  const scale = interpolate(p, [0, 1], [0, 1], { extrapolateRight: "clamp" });
  const opacity = interpolate(p, [0, 1], [0, 1], { extrapolateRight: "clamp" });
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" style={{ transform: `scale(${scale})`, opacity, filter: "drop-shadow(0 0 14px rgba(224,131,79,0.55))" }}>
      <path
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", opacity, transform: `translateY(${y}px)` }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 10 }}>
        <Star size={40} delay={born + 6} frame={frame} fps={fps} />
        <Star size={64} delay={born} frame={frame} fps={fps} />
        <Star size={40} delay={born + 6} frame={frame} fps={fps} />
      </div>
    </div>
  );
